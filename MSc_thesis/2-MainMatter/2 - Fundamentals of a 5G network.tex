...
This means that network operators can dimension their networks to adapt to real network traffic patterns, deploying control plane functions in centralized locations for efficient signaling, while user plane functions can be distributed closer to the network edge to minimize service latency and jittering, improving throughput and overall user experience.
...
The adoption of 5G technology has been a gradual process for many network operators, as the advent of 5G networks, while innovative, also meant a significant revamp in existing infrastructure, which brought along considerable costs, logistical challenges and unavoidable service disruptions.
...
The 5G \glsxtrlong{RAN} constitutes the interface between \glspl{UE} and the 5G Core network, providing the radio connectivity and resource management functions essential to mobile communication. It encompasses the physical, MAC and higher layers of the protocol stack, enabling data transmission, mobility management and strict service differentiation. 
...
The \gls{NG-RAN}, compared to its predecessors, has undergone extensive softwarization and disaggregation of the RAN. This archirectural shift replaces fixed hardware entities with modular components and virtualized functions that can be deployed in cloud-native environments. As a result, in this generation, the \gls{RAN} acts as a dynamic platform that interacts closely with the 5G Core network and its functions to deliver optimized connectivity and services.
...
The \gls{gNB} is further divided into three main primary units: 
\begin{description}
...
\end{figure}
As depicted in Figure~\ref{fig:5g_ran_interfaces}, the 5G \gls{RAN} utilizes standardized interfaces to facilitate communication between its components and the 5G Core network. The primary interfaces include:
\begin{description}
  ...
  The Xn interface connects a \glspl{gNB} between each other, enabling inter-cell coordination and handover management. This interface also supports dual connectivity scenarios, where a \gls{UE} can be simultaneously connected to multiple \glspl{gNB} for improved performance and reliability. 
...
\begin{itemize}
  ...
  \item \textbf{Frequency Range 2 (FR2)}: Known as \glsxtrshort{mmWave}, it covers frequencies from 24.25 GHz to 71 GHz. This range greatly increases available bandwidth and, therefore, throughput, but suffers from significant path loss. It is important to note that many \glsxtrshort{COTS} devices are not compatible with the frequency range.
\end{itemize}
\pagebreak
...
% TODO rewrite in own words: contrast 5G's unified/SUCI-based authentication (via AUSF/UDM) against 4G's IMSI-in-the-clear approach
As introduced in Section~\ref{sec:5g_security}, the \gls{AUSF} and \gls{UDM} together embody one of 5G's key security improvements over 4G: a unified authentication framework capable of supporting both \gls{3GPP} and non-\gls{3GPP} access (the latter via the \gls{N3IWF}), combined with subscriber identity concealment, where the \gls{UE}'s permanent identifier (\gls{SUPI}) is encrypted into a \gls{SUCI} before being sent over the air, unlike 4G, where the \gls{IMSI} could be exposed in the clear during initial attach.
