...
\begin{itemize}
    ...
    \item Diagnose configuration issues surfaced during testing, documenting the reasoning behind each change rather than only the final configuration.
    \item Leave a working, documented 4G and 5G testbed in place for future research and coursework at \gls{FCT}.
\end{itemize}

The contributions expected from this dissertation are as follows:
\begin{itemize}
    ...
    \item An empirical and quantitative multi-round comparison of 4G and 5G performance, throughput, latency and reliability, under matched distance and traffic conditions.
    \item A documented report of how specific configuration issues were diagnosed and corrected across successive test rounds, intended as a practical reference for others deploying a similar open-source stack.
\end{itemize}
...
\begin{itemize}
    ...
    \item Chapter~\ref{cha:proposed-architecture} presents the implementation of the testbed: the hardware components, the core network design, the \gls{RAN} design, and the interfaces connecting them.
    \item Chapter~\ref{cha:methodology} details the methodology used to evaluate it across three successive test rounds and the changes made to the 5G \gls{RAN} configuration.
    ...
    \item Chapter~\ref{cha:conclusions} concludes the dissertation, summarizing the main findings, their limitations, and the work left open for the future.
\end{itemize}