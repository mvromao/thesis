%!TEX root = ../template.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Config/7_package.tex
%% NOVA thesis configuration file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\typeout{NT FILE Config/5_packages.tex}%

% \ntlangsetup{pt/contentsname=O MEU ÍNDICE}
% \ntlangsetup{en/contentsname=MY LITTLE TOC}

%%============================================================
%%  FOR DEMO PURPOSES ONLY --- YOU SHOULD REMOVE THESE PACKAGES
%%============================================================
\usepackage{float}        % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{wrapfig}      % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\restylefloat{figure}     % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{metalogo}     % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{lipsum}       % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\IfFileExists{byo-twemojiss.sty}{
  \usepackage{byo-twemojis}     % to typeset emojis
  \newcommand{\emojiSmile}{\raisebox{-3pt}{\scalebox{1.3}{\byoTwemoji{head; eyes normal opened; mouth laughing}}}}
}{
  \newcommand{\emojiSmile}{\texttt{:D}}
  \newlength{\twemojiDefaultHeight}
}

\newcommand{\MikTeX}{Mik\TeX}
\newcommand{\TeXLive}{\TeX Live}
\newcommand{\pdfLaTeX}{pdf\LaTeX}
\definecolor{Schlgray}{gray}{0.95}

\usetikzlibrary{decorations.pathreplacing}  % for the TDD structure diagram's braces


%%============================================================
%%  ADD YOUR PACKAGES AND OTHER CUSTOMIZATIONS HERE
%%============================================================

\raggedbottom  % let pages end at their natural height instead of stretching
               % interparagraph space to fill the page (memoir defaults to
               % \flushbottom, which was causing visible gaps before floats)

%CHANGED BY CLAUDE: left-align short captions too. By default memoir centres a caption that
% fits on one line and justifies longer ones, so alignment depended on caption length.
% Delete this line to go back to the default.
\captionstyle[\raggedright]{}

%CHANGED BY CLAUDE: number the appendix chapters (A, B, ...). NOVAthesis switches to
% back matter (\@mainmatterfalse) before loading the appendices, and memoir does not
% number chapters outside the main matter, so references to them rendered as ".7".
\makeatletter
\pretocmd{\ntprintappendices}{\@mainmattertrue}{}{\typeout{WARNING: appendix numbering patch failed}}
\apptocmd{\ntprintappendices}{\@mainmatterfalse}{}{\typeout{WARNING: appendix numbering patch failed}}
\makeatother

% \usepackage[orphans/prevent, widows/prevent]{widows-and-orphans} % add warnings about widows and orphans to “template.log”
% \usepackage{refcheck}     % list used and unused labels

\usepackage{tabularx}

\usepackage{paralist}     % To enable costumizable enumerates.  See the documentation.

\usepackage[textsize=footnotesize]{todonotes}  % To register TODO notes in the text

\usepackage{makecell}      % Multiline table cells and control vertical and horizontal alignment

\usepackage{listings}

\usepackage{tcolorbox}    % pretty print color boxes
\tcbuselibrary{listings}

\newtcblisting{code}{
  colback=gray!20,
  colframe=black,
  boxrule=1pt,
  boxsep=0pt,
  arc=0pt,
  listing only,            % Only shows the code, no output box
  listing options={
    breaklines=true,       % This enables the word wrap
    breakatwhitespace=true, % Prevents breaking in the middle of words
    basicstyle=\ttfamily\small,
    columns=fullflexible,
    extendedchars=true,
    literate={ }{ }1
  }
}

\usepackage[table]{xcolor}

\usepackage{xcolor}

\lstset{
  basicstyle=\ttfamily,
  columns=fullflexible,
  breaklines=true,
  escapeinside={(*}{*)} % This defines the escape delimiters
}

% Matches pygments' default style for YAML (as used by minted elsewhere in this thesis):
% mapping keys in bold green, everything else left as plain text.
\definecolor{yamlkeycolor}{rgb}{0.00,0.50,0.00}
\lstdefinelanguage{yaml}{
  keywords={tdd_ul_dl_cfg,dl_ul_tx_period,nof_dl_slots,nof_dl_symbols,nof_ul_slots,nof_ul_symbols},
  keywordstyle=\color{yamlkeycolor}\bfseries,
  sensitive=true,
  comment=[l]{\#},
  commentstyle=\itshape\color{gray},
  morestring=[b]",
  morestring=[b]',
  stringstyle=\color{red!70!black},
}


% \usepackage{indentfirst}       % Indent first list after a (sub)section title

% \usepackage{algorithm2e}
% \RestyleAlgo{ruled}

\usepackage{minted}    % Fontification of source code listings

% \usepackage{listings}    % Fontification of source code listingsConfig/5_packages.tex
%                          % In alternative, use the package “minted” (highly recomended)

% \lstset{
%     captionpos=t,
%     basicstyle={\ttfamily\footnotesize},
%     numbers=left,
%     numberstyle={\ttfamily\tiny},
%     tabsize=2,
%     language=Java,
%     float,
%     frame=single,
%     columns=fullflexible,
%     breaklines=true,
%     postbreak=\mbox{\textcolor{red}{$\hookrightarrow$}\space},
%     inputencoding=utf8,
%     extendedchars=true,
%     literate=
%       {á}{{\'a}}1 {é}{{\'e}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {ú}{{\'u}}1
%       {Á}{{\'A}}1 {É}{{\'E}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {Ú}{{\'U}}1
%       {à}{{\`a}}1 {è}{{\`e}}1 {ì}{{\`i}}1 {ò}{{\`o}}1 {ù}{{\`u}}1
%       {À}{{\`A}}1 {È}{{\'E}}1 {Ì}{{\`I}}1 {Ò}{{\`O}}1 {Ù}{{\`U}}1
%       {ä}{{\"a}}1 {ë}{{\"e}}1 {ï}{{\"i}}1 {ö}{{\"o}}1 {ü}{{\"u}}1
%       {Ä}{{\"A}}1 {Ë}{{\"E}}1 {Ï}{{\"I}}1 {Ö}{{\"O}}1 {Ü}{{\"U}}1
%       {â}{{\^a}}1 {ê}{{\^e}}1 {î}{{\^i}}1 {ô}{{\^o}}1 {û}{{\^u}}1
%       {Â}{{\^A}}1 {Ê}{{\^E}}1 {Î}{{\^I}}1 {Ô}{{\^O}}1 {Û}{{\^U}}1
%       {œ}{{\oe}}1 {Œ}{{\OE}}1 {æ}{{\ae}}1 {Æ}{{\AE}}1 {ß}{{\ss}}1
%       {ç}{{\c c}}1 {Ç}{{\c C}}1 {ø}{{\o}}1 {å}{{\r a}}1 {Å}{{\r A}}1
%       {€}{{\EUR}}1 {£}{{\pounds}}1
% }

